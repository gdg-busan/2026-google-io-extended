...
/** Card moderation (task #6): hidden toggle. Admin SDK — rules deny client writes to this field. */
export async function PATCH(request: Request, { params }: RouteParams) {
  ...
  await adminDb.collection("cards").doc(id).update({ hidden: parsed.data.hidden });
